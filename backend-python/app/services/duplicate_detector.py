...
Detects duplicate bank statements using file and content hashes.
"""
import logging
...
class DuplicateCheckResult:
    ...
    def __init__(
        ...
        self.duplicate_type = duplicate_type  # "exact_file", "exact_content"
...
def check_for_duplicates(
    ...
    """
    Duplicate detection using file hash and content hash.
    ...
    for stmt in existing_statements:
        if stmt.get("file_hash") == file_hash:
            LOGGER.warning(
                f"Exact file duplicate detected: {file_path.name} matches {stmt.get('filename')}"
            )
            return DuplicateCheckResult(
                is_duplicate=True,
                ...
                message=(
                    f"This file is an exact duplicate of '{stmt.get('filename')}' "
                    "uploaded previously."
                ),
                file_hash=file_hash,
                content_hash=content_hash,
    ...
    for stmt in existing_statements:
        if stmt.get("content_hash") == content_hash:
            LOGGER.warning(
                f"Exact content duplicate detected: {file_path.name} matches {stmt.get('filename')}"
            )
            return DuplicateCheckResult(
                is_duplicate=True,
                ...
                original_filename=stmt.get("filename"),
                message=(
                    f"This statement contains identical transaction data to '{stmt.get('filename')}' "
                    "(possibly rescanned or renamed)."
                ),
                file_hash=file_hash,
                content_hash=content_hash,
            )
...
def _get_recommendation(result: DuplicateCheckResult) -> str:
    ...
    if result.duplicate_type == "exact_file":
        return (
            "This is an exact copy of a previously uploaded file. "
            "Processing it will create duplicate transactions."
        )

    if result.duplicate_type == "exact_content":
        return (
            "This statement contains identical transaction data to a previous upload. "
            "It may be a rescanned or renamed version of the same statement."
        )
